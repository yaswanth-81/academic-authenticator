#!/usr/bin/env python3
"""
Final Certificate OCR Text Extractor
Extracts structured information from academic certificate OCR text with maximum accuracy
    ...
    for line in lines:
        ...
        if not fields["Student Name"]:
            if "STUDENT NAME" in line_upper:
                value = extract_value_after_keyword(line, "STUDENT NAME")
                if value:
        ...
        if not fields["Hall Ticket No"]:
            if "HALL TICKET" in line_upper:
                value = extract_value_after_keyword(line, "HALL TICKET")
                if value:
        ...
        if not fields["Roll Number"]:
            if "ROLL NUMBER" in line_upper:
                value = extract_value_after_keyword(line, "ROLL NUMBER")
                if value:
        ...
        if not fields["University Name"]:
            if "UNIVERSITY NAME" in line_upper:
                value = extract_value_after_keyword(line, "UNIVERSITY NAME")
                if value:
        ...
        if not fields["College Name"]:
            if "COLLEGE NAME" in line_upper:
                value = extract_value_after_keyword(line, "COLLEGE NAME")
                if value:
        ...
        if not fields["Branch"]:
            if "BRANCH" in line_upper:
                value = extract_value_after_keyword(line, "BRANCH")
                if value:
...
def extract_value_after_keyword(line: str, keyword: str) -> Optional[str]:
    """Extract value after a keyword in a line"""
